...
function CountryCard({ country, onRemove }) {
    return (
        <div className="card" onDoubleClick={() => onRemove(country.id)}>
...
function App() {
    ...
    const [countries, setCountries] = useState(getRandomCountries());
    ...
    return (
        <div>
            <h1 style={{ textAlign: "center" }}>CountryPedia</h1>
            <div className="container">
                {countries.map(country => (
                    <CountryCard key={country.id} country={country} onRemove={removeCountry} />
                ))}
            </div>
            <footer>Total Population: {totalPopulation.toLocaleString()}</footer>
        </div>
    );
}
